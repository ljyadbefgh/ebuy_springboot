...
public class AdminServiceImpl implements AdminService {
    ...
    @Override
    public boolean login(String username, String password) throws MyWebException {
        boolean judge=false;
    ...
    @Override
    public void deleteAdmin(Admin admin,Integer id) throws MyWebException {
        if(admin.getId()==id.intValue()) {//如果登录账户的id与被删除账户的id一致
            throw new MyWebException("删除失败：不允许删除自己的账户");
        }else{
            adminDao.delete(id);
        }
    }
    ...
    @Override
    public void deleteAdmins(Admin admin,Integer[] ids)  throws MyWebException {
        //先进行验证
        for(Integer id:ids){
            if(admin.getId()==id.intValue()) {//如果登录账户的id与被删除账户的id一致
                throw new MyWebException("删除失败：不允许删除自己的账户");
    ...
    @Override
    public void addAdmin(Admin admin) throws MyWebException {
        if(admin!=null){
            ...
            if(adminDao.countUsername(admin.getUsername())==0){
                ...
                throw new MyWebException("账户添加失败：账户名重名");
            }
        }else{
    ...
    @Override
    public boolean updateAdmin(Admin admin) throws MyWebException {
        ...
        if(admin.getUsername().length()==0){
            ...
            throw new MyWebException("账户编辑失败：和其他管理账户的账户名重名");
        }else{
